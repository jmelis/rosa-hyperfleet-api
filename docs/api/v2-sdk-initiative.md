...
2. **Connection / auth / transport**: The hand-written connection builder, AWS SigV4 authentication, retry behavior, and transport configuration that the generated client uses to talk to the Platform API.
...
Two options for the consumer-facing interface:

**Option A: V1-compatible interface** — consumers use identical import paths and method signatures. A build tag or connection config switches between v1 and v2 backend. Lowest migration cost for rosa/terraform, but constraining long-term.
...
- Tenancy and authz: account linking, policies, attachments, authorization checks (future iteration)
...
3. **Initial surface**: Cluster + NodePool only. Tenancy, authz, and account linking are out of scope for this iteration.
...
4. **Type compatibility**: How much do the HyperFleet CRD types (Cluster, NodePool) diverge from the OCM API model types? The migration complexity depends on this.
